...
function checkCoordinateString(coordString) {
    // Regular expression to match the coordinate format
    const regex = /^([NS]\d{4}[EW]\d{5})(\s-\s[NS]\d{4}[EW]\d{5})+$/;

    // Test if the string matches the regex
    if (regex.test(coordString.trim())) {
        return coordString;
    } else {
        return "";
    }
...
function cutPlotFIRs(poly, plotar = true) {
    let coordEdit
    ...
    if (!poly) {
        ...
        if (typeof (poly) == "string") {
            coordEdit = checkCoordinateString(poly)
            if (coordEdit == "")
                return []
        }
        else { //tipo layer
    ...
    //limpa array global dos recortados
    arrCutted = []

    let firsEncontradas = [];

    // Verifica cada FIR e adiciona ao array se encontrada
    if (cutPlotFIR(coordEdit, [getCoordDegAirmet(firAZCoords)], descricao, plotar)) {
        firsEncontradas.push('SBAZ');
    }

    if (cutPlotFIR(coordEdit, [getCoordDegAirmet(firAOCoords)], descricao, plotar)) {
        firsEncontradas.push('SBAO');
    }

    if (cutPlotFIR(coordEdit, [getCoordDegAirmet(firRECoords)], descricao, plotar)) {
        firsEncontradas.push('SBRE');
    }

    if (cutPlotFIR(coordEdit, [getCoordDegAirmet(firBSCoords)], descricao, plotar)) {
        firsEncontradas.push('SBBS');
    }

    if (cutPlotFIR(coordEdit, [getCoordDegAirmet(firCWCoords)], descricao, plotar)) {
        firsEncontradas.push('SBCW');
    }

    // Se mais de uma FIR foi encontrada, limpa o texto das coordenadas no navegador e a area de transferencia
    if (firsEncontradas.length > 1) {
        copiaCoordenadas(" ", true);
    }
    return firsEncontradas
}
...
function cutPlotFIR(coordEdit, tPolyFir, descricao, plotar = true) {
    //    let tPolyFir = [getCoordDegAirmet(coordFIR)]
    // let tPolyFir = coordFIR
    function verificaAreas(polyEdit ,plotFirCut){
        let areaCortes = turf.area(plotFirCut)
        let areaTotal = turf.area(polyEdit)
        let percentual
        
        percentual = Math.round((areaCortes/areaTotal) * 100)
        // Erro se mais de 2% da área estiver fora da FIR
        if (percentual > 50 &&  percentual < 98) { 
            console.log ("Area Total: ",areaTotal)
            console.log ("Percentual da Área Cortada: ",percentual,"%")
        }
    ...
    let polyFir,polyEdit,plotFirCut,areaTotal
    try {
        polyFir = turf.polygon(tPolyFir)

        polyEdit = turf.polygon([getCoordDegAirmet(coordEdit)]);


        plotFirCut = turf.intersect(polyFir, polyEdit);
        verificaAreas(polyEdit, plotFirCut)
    ...
    if (plotFirCut) {
        if (plotar) {
            ...
            for (let i in iCoord) {
                iPoly = iCoord[i][0]
                let coord = invertLatLong(iPoly)
                //let areaCortado = getArea(turf.Pcoord)
